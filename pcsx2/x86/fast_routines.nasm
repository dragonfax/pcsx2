...
%ifdef __APPLE__
	%define g_EEFreezeRegs _g_EEFreezeRegs 
  %define FreezeMMXRegs_ _FreezeMMXRegs_
	%define memcmp_mmx _memcmp_mmx
	%define memxor_mmx _memxor_mmx
	%define _mmx_backup __mmx_backup
	%define memcpy_amd_ _memcpy_amd_
%else	
	%define g_EEFreezeRegs g_EEFreezeRegs 
  %define FreezeMMXRegs_ FreezeMMXRegs_
	%define memcmp_mmx memcmp_mmx
	%define memxor_mmx memxor_mmx
	%define _mmx_backup _mmx_backup
	%define memcpy_amd memcpy_amd	
%endif
...
global memcmp_mmx
memcmp_mmx:
; make sure regs are stored
; FreezeMMXRegs(1);
;	cmp dword [g_EEFreezeRegs],0
;	jz memcmp_mmx_begin
;	push 1
;	call FreezeMMXRegs_
;	add esp,4
...
memcmp_mmx_begin:		
	push esi
	mov MEMCMP_SRC1, dword [esp+8]
	mov MEMCMP_SRC2, dword [esp+12]
	mov MEMCMP_SIZE, dword [esp+16]

	cmp MEMCMP_SIZE, 32
	jl near memcmp_Done4
...
; check if eq
	cmp eax, 0xff
	je near memcmp_ContinueTest
	mov eax, 1
	jmp memcmp_End

	cmp MEMCMP_SIZE, 64
	jl near memcmp_Done8
...
memxor_mmx_begin:
	push esi
	mov MEMXOR_SRC1, dword [esp+8]
	mov MEMXOR_SRC2, dword [esp+12]
	mov MEMXOR_SIZE, dword [esp+16]
	cmp MEMXOR_SIZE, 64
	jl near memxor_Setup4
...
memcpy_align_done:	;		// destination is dword aligned
	...
	jz		near memcpy_ic_2	;// finish the last few bytes

	mov   edx, _mmx_backup ;// will probably need this to save/restore mmx
	cmp		eax, IN_CACHE_COPY/64	;// too big 4 cache? use uncached copy
	jae		near memcpy_uc_test
...
memcpy_ic_1:			;// 64-byte block copies, in-cache copy
	...
	dec		eax				;// count down
	jnz		near memcpy_ic_1	;// last 64-byte block?
...
memcpy_ic_3:
	...
	and		eax, 0xF		;// only look at the "remainder" bits
	neg		eax				;// set up to jump into the array
	add		eax, memcpy_last_few
	jmp		eax				;// jump to array of movsd's

memcpy_uc_test:
	or		eax, eax		;// tail end of block prefetch will jump here
	jz		near memcpy_ic_2 ;	// no more 64-byte blocks left
...
memcpy_last_few:		;// dword aligned from before movsd's
	and		ecx, 0x3	;// the last few cows must come home
	jz		near memcpy_final	;// no more, let's leave
